...
class Particle(arcade.SpriteCircle):
    def __init__(self, radius, color, change_x, change_y):
...
class HUD:
    ...
    def update_text(self, coins, deaths, jumps):
        self.score_text.text = f"Монеты: {coins} | Смерти: {deaths} | Прыжки: {jumps}"
...
class GameView(arcade.View):
    ...
    def check_collisions(self):
        ...
        for coin in arcade.check_for_collision_with_list(self.player_sprite, self.coins_list):
            ...
            self.create_particles(coin.center_x, coin.center_y, arcade.color.GOLD)
        ...
        if arcade.check_for_collision_with_list(self.player_sprite, self.enemies_list) or \
            ...
            self.create_particles(self.player_sprite.center_x, self.player_sprite.center_y, arcade.color.RED)
    ...
    def on_update(self, delta_time: float):
        ...
        ice_hit_list = arcade.check_for_collision_with_list(self.player_sprite, self.ice_list)
        self.player_sprite.center_y += 2
        on_ice = len(ice_hit_list) > 0
        ...
        self.sync_camera()
        self.interface.update_text(self.coins_collected, self.deaths_total, self.jumps_made)
    ...
    def create_particles(self, x, y, color):
        for i in range(20):
            particle = Particle(
                ...
                color=color,
                change_x=random.uniform(-3, 3),
                change_y=random.uniform(-3, 3)
            )
            particle.center_x = x
            particle.center_y = y
...
class Level2View(GameView):
    def __init__(self, deaths=0, coins=0, superpower=False):
        super().__init__(deaths)
        self.coins_collected = coins
        self.has_superpower = superpower
        self.message = ""
    ...
    def check_collisions(self):
        ...
        if arcade.check_for_collision_with_list(self.player_sprite, self.trap_list) or \
                arcade.check_for_collision_with_list(self.player_sprite, self.water_list):
            self.deaths_total += 1
            self.stop_walk_sound()

            self.window.show_view(GameOverLevel2View(self.deaths_total, self.coins_collected, self.has_superpower))
            return

        if arcade.check_for_collision_with_list(self.player_sprite, self.enemies_list):
            self.deaths_total += 1
            self.stop_walk_sound()
            self.window.show_view(GameOverLevel2View(self.deaths_total, self.coins_collected, self.has_superpower))
            return
        ...
        if key == arcade.key.Q:
            if self.has_superpower:
                ...
                for enemy in self.enemies_list:
                    if arcade.get_distance_between_sprites(self.player_sprite, enemy) < 250:
                        self.create_particles(enemy.center_x, enemy.center_y, arcade.color.ELECTRIC_BLUE)
                        enemy.remove_from_sprite_lists()
                        killed = True
            else:
                self.message = "У ВАС НЕТ СУПЕРСИЛЫ!"
    ...
    def on_draw(self):
        ...
        if self.message_timer > 0:
            arcade.draw_text(self.message, self.window.width / 2, self.window.height / 2 + 50,
                             arcade.color.RED, 22, bold=True, anchor_x="center")


class GameOverLevel2View(GameOverView):
    def __init__(self, death_count, coins, superpower):
        super().__init__(death_count)
        self.coins = coins
        self.superpower = superpower

    def on_key_press(self, key, modifiers):
        if key == arcade.key.ENTER:
            game = Level2View(deaths=self.death_count, coins=self.coins, superpower=self.superpower)
            game.setup()
            self.window.show_view(game)
...
class BetweenLevelsView(BaseView):
    ...
    def on_draw(self):
        self.clear()
        shop_text = f"\n\nУ вас монет: {self.coins}"

        if not self.has_superpower:
            shop_text += "\nНажмите 'S', чтобы купить СУПЕРСИЛУ (3 монеты)"
            shop_text += "\n(Позволяет убивать врагов нажатием на 'Q')"
        else:
            shop_text += "\nСУПЕРСИЛА КУПЛЕНА!"
            shop_text += "\nНа Уровне 2 нажимайте 'Q' рядом с врагами, чтобы уничтожить их!"

        self.display_message(f"УРОВЕНЬ 1 ПРОЙДЕН!{shop_text}\n\nНажмите ENTER, чтобы начать Уровень 2", 0, 20,
                             arcade.color.WHITE)

        if self.message_timer > 0:
            arcade.draw_text(self.message, self.window.width / 2, 100,
                             arcade.color.RED, 20, anchor_x="center")

    def on_key_press(self, key, modifiers):
        if key == arcade.key.S and not self.has_superpower:
            if self.coins >= 3:
                self.coins -= 3
                self.has_superpower = True
                self.message = "СУПЕРСИЛА КУПЛЕНА!"
                self.message_timer = 2.0
            else:
        ...
        if key == arcade.key.ENTER:
            lvl2 = Level2View(deaths=self.deaths, coins=self.coins, superpower=self.has_superpower)
            lvl2.setup()
